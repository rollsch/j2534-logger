...
static void EnumPassThruInterfaces(std::set<cPassThruInfo> &registryList)
{
	...
	// Open HKLM/Software
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\PassThruSupport.04.04"), 0, KEY_READ, &hKey1) != ERROR_SUCCESS)
	{
		//TODO add auto 32/64 bit detection correctly, above seems to auto detect WOW6432 on win10
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, _T("Software\\WOW6432Node\\PassThruSupport.04.04"), 0, KEY_READ, &hKey1) != ERROR_SUCCESS)
		{
			//If we cant open the key exit
			return;
		}
	...
	DWORD lMaxSubKeyLen;
	RegQueryInfoKey(hKey1, NULL, NULL, NULL, NULL, &lMaxSubKeyLen, NULL, NULL, NULL, NULL, NULL, NULL);
	...
	do
	{
		...
		hKey2RetVal = RegEnumKeyEx(hKey1, VendorIndex++, KeyValue, &KeySize, NULL, NULL, NULL, &FTime);
		...
		if (RegOpenKeyEx(hKey1, KeyValue, 0, KEY_READ, &hKey3) == ERROR_SUCCESS)
		{
